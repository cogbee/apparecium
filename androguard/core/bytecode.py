...
def printhelper(arr, el):
    if arr is not None and arr.get(el) is not None:
        return " ".join(arr.get(el)[1])
    else:
        return ""

def printhelperannotations(annotations, ins_idx):
    if annotations is not None and annotations.get(ins_idx) is not None:
        s = ""
        for k, v in annotations.get(ins_idx).iteritems():
            s = "%s %s: %s<br/>" % (s, k, " ".join(v))
        return s
        #return " ".join(annotations.get(ins_idx)[1])
    else:
        return ""


def method2dot(mx, backward, forward, combined, annotations, types, types_total, colors={}):
    """
    ...
    for DVMBasicMethodBlock in mx.basic_blocks.gets():
        ...
        for DVMBasicMethodBlockInstruction in DVMBasicMethodBlock.get_instructions():
            ...
            content += label_tpl % (bg_idx,
                                    ...
                                    output,
                                    printhelper(forward, ins_idx),
                                    printhelper(backward, ins_idx),
                                    printhelper(combined, ins_idx),
